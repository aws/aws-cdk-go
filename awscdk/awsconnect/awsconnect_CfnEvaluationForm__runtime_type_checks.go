		...
		for idx_97dfc6, v := range *val {
			...
			default:
				if !_jsii_.IsAnonymousProxy(v) {
					return fmt.Errorf("parameter val[%#v] must be one of the allowed types: *CfnEvaluationForm_EvaluationFormBaseItemProperty, awscdk.IResolvable; received %#v (a %T)", idx_97dfc6, v, v)
				}
			}
		}
	case []interface{}:
...
func (j *jsiiProxy_CfnEvaluationForm) validateSetScoringStrategyParameters(val interface{}) error {
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: *CfnEvaluationForm_ScoringStrategyProperty, awscdk.IResolvable; received %#v (a %T)", val, val)
		}
	}
