...
// Lambda handler code as an S3 object.
func InlineCode_FromBucketV2(bucket awss3.IBucket, key *string, options *BucketOptions) S3CodeV2 {
	_init_.Initialize()

	if err := validateInlineCode_FromBucketV2Parameters(bucket, key, options); err != nil {
		panic(err)
	}
...
func InlineCode_FromCustomCommand(output *string, command *[]*string, options *CustomCommandOptions) AssetCode {
	...
	if err := validateInlineCode_FromCustomCommandParameters(output, command, options); err != nil {
		panic(err)
	}
	var returns AssetCode
