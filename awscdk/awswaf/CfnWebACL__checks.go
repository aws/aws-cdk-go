...
func validateCfnWebACL_ArnForWebACLParameters(resource interfacesawswaf.IWebACLRef) error {
	if resource == nil {
...
func (j *jsiiProxy_CfnWebACL) validateSetDefaultActionParameters(val interface{}) error {
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: awscdk.IResolvable, *CfnWebACL_WafActionProperty; received %#v (a %T)", val, val)
		}
	}
		...
		for idx_97dfc6, v := range *val {
			...
			default:
				if !_jsii_.IsAnonymousProxy(v) {
					return fmt.Errorf("parameter val[%#v] must be one of the allowed types: awscdk.IResolvable, *CfnWebACL_ActivatedRuleProperty; received %#v (a %T)", idx_97dfc6, v, v)
				}
			}
		}
	case []interface{}:
