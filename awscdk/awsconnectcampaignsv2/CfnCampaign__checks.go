...
func (j *jsiiProxy_CfnCampaign) validateSetEntryLimitsConfigParameters(val interface{}) error {
	...
	case awscdk.IResolvable:
		// ok
	case *CfnCampaign_EntryLimitsConfigProperty:
		val := val.(*CfnCampaign_EntryLimitsConfigProperty)
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	case CfnCampaign_EntryLimitsConfigProperty:
		val_ := val.(CfnCampaign_EntryLimitsConfigProperty)
		val := &val_
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: awscdk.IResolvable, *CfnCampaign_EntryLimitsConfigProperty; received %#v (a %T)", val, val)
		}
	}
