...
func (j *jsiiProxy_CfnJobDefinition) validateSetResourceRetentionPolicyParameters(val interface{}) error {
	...
	case awscdk.IResolvable:
		// ok
	case *CfnJobDefinition_ResourceRetentionPolicyProperty:
		val := val.(*CfnJobDefinition_ResourceRetentionPolicyProperty)
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	case CfnJobDefinition_ResourceRetentionPolicyProperty:
		val_ := val.(CfnJobDefinition_ResourceRetentionPolicyProperty)
		val := &val_
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: awscdk.IResolvable, *CfnJobDefinition_ResourceRetentionPolicyProperty; received %#v (a %T)", val, val)
		}
	}
