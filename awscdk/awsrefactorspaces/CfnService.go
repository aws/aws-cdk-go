...
func (j *jsiiProxy_CfnService)SetLambdaEndpoint(val interface{}) {
	if err := j.validateSetLambdaEndpointParameters(val); err != nil {
		panic(err)
	}
	_jsii_.Set(
		j,
...
func (j *jsiiProxy_CfnService)SetUrlEndpoint(val interface{}) {
	if err := j.validateSetUrlEndpointParameters(val); err != nil {
		panic(err)
	}
	_jsii_.Set(
		j,
...
func CfnService_ArnForService(resource interfacesawsrefactorspaces.IServiceRef) *string {
	_init_.Initialize()
