...
// May contain strings returned by `EventField.from()` to substitute in parts of the
// matched event.
func LogGroupTargetInput_FromObjectV2(options *LogGroupTargetInputOptions) LogGroupTargetInput {
	_init_.Initialize()

	if err := validateLogGroupTargetInput_FromObjectV2Parameters(options); err != nil {
		panic(err)
	}
	var returns LogGroupTargetInput
