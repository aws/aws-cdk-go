...
func (j *jsiiProxy_CfnCluster) validateSetClusterEndpointsParameters(val interface{}) error {
	switch val.(type) {
	...
	case *[]interface{}:
		...
		for idx_97dfc6, v := range *val {
			switch v.(type) {
			case *CfnCluster_ClusterEndpointProperty:
				v := v.(*CfnCluster_ClusterEndpointProperty)
				if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
					return err
				}
			case CfnCluster_ClusterEndpointProperty:
				v_ := v.(CfnCluster_ClusterEndpointProperty)
				v := &v_
				if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
					return err
				}
			...
			default:
				if !_jsii_.IsAnonymousProxy(v) {
					return fmt.Errorf("parameter val[%#v] must be one of the allowed types: *CfnCluster_ClusterEndpointProperty, awscdk.IResolvable; received %#v (a %T)", idx_97dfc6, v, v)
				}
			}
		}
	case []interface{}:
