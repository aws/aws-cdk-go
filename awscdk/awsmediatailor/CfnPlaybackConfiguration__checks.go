...
func (j *jsiiProxy_CfnPlaybackConfiguration) validateSetLogConfigurationParameters(val interface{}) error {
	switch val.(type) {
	case awscdk.IResolvable:
		// ok
	case *CfnPlaybackConfiguration_LogConfigurationProperty:
		val := val.(*CfnPlaybackConfiguration_LogConfigurationProperty)
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	case CfnPlaybackConfiguration_LogConfigurationProperty:
		val_ := val.(CfnPlaybackConfiguration_LogConfigurationProperty)
		val := &val_
		if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
			return err
		}
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: awscdk.IResolvable, *CfnPlaybackConfiguration_LogConfigurationProperty; received %#v (a %T)", val, val)
		}
	}
