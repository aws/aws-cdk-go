...
func Stack_ConsumeListReference(value *[]*string, strength ReferenceStrength) *[]*string {
	_init_.Initialize()

	if err := validateStack_ConsumeListReferenceParameters(value); err != nil {
		panic(err)
	}
	var returns *[]*string
...
//
// Returns: A token that resolves to the same value but uses the overridden strength.
func Stack_ConsumeReference(value *string, strength ReferenceStrength) *string {
	_init_.Initialize()

	if err := validateStack_ConsumeReferenceParameters(value); err != nil {
		panic(err)
	}
	var returns *string
...
func (s *jsiiProxy_Stack) With(mixins ...constructs.IMixin) constructs.IConstruct {
	args := []interface{}{}
