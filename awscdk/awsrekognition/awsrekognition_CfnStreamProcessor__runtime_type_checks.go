...
func (j *jsiiProxy_CfnStreamProcessor) validateSetPolygonRegionsOfInterestParameters(val interface{}) error {
	...
	case *[]interface{}:
		...
		for idx_97dfc6, v := range *val {
			switch v.(type) {
			case *CfnStreamProcessor_PolygonProperty:
				v := v.(*CfnStreamProcessor_PolygonProperty)
				if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
					return err
				}
			case CfnStreamProcessor_PolygonProperty:
				v_ := v.(CfnStreamProcessor_PolygonProperty)
				v := &v_
				if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
					return err
				}
			...
			default:
				if !_jsii_.IsAnonymousProxy(v) {
					return fmt.Errorf("parameter val[%#v] must be one of the allowed types: *CfnStreamProcessor_PolygonProperty, awscdk.IResolvable; received %#v (a %T)", idx_97dfc6, v, v)
				}
			}
		}
	case []interface{}:
