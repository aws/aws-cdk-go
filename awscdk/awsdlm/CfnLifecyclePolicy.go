...
func (j *jsiiProxy_CfnLifecyclePolicy)SetCrossRegionCopyTargets(val interface{}) {
	if err := j.validateSetCrossRegionCopyTargetsParameters(val); err != nil {
		panic(err)
	}
	_jsii_.Set(
		j,
...
func CfnLifecyclePolicy_ArnForLifecyclePolicy(resource interfacesawsdlm.ILifecyclePolicyRef) *string {
	_init_.Initialize()

	if err := validateCfnLifecyclePolicy_ArnForLifecyclePolicyParameters(resource); err != nil {
		panic(err)
	}
	var returns *string
