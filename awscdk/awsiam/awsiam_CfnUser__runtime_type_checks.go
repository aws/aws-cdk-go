...
func (j *jsiiProxy_CfnUser) validateSetLoginProfileParameters(val interface{}) error {
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: *CfnUser_LoginProfileProperty, awscdk.IResolvable; received %#v (a %T)", val, val)
		}
	}
		...
		for idx_97dfc6, v := range *val {
			...
			default:
				if !_jsii_.IsAnonymousProxy(v) {
					return fmt.Errorf("parameter val[%#v] must be one of the allowed types: *CfnUser_PolicyProperty, awscdk.IResolvable; received %#v (a %T)", idx_97dfc6, v, v)
				}
			}
		}
	case []interface{}:
