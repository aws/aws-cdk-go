package previewawsb2bievents

import (
	_init_ "github.com/aws/aws-cdk-go/awscdkmixinspreview/v2/jsii"
	_jsii_ "github.com/aws/jsii-runtime-go/runtime"

	"github.com/aws/aws-cdk-go/awscdk/v2/awsevents"
)

// EventBridge event pattern for aws.b2bi@AcknowledgementFailed.
//
// Example:
//   // The code below shows an example of how to instantiate this type.
//   // The values are placeholders you should change.
//   import "github.com/aws/aws-cdk-go/awscdkmixinspreview"
//
//   acknowledgementFailed := awscdkmixinspreview.Events.NewAcknowledgementFailed()
//
// Experimental.
type AcknowledgementFailed interface {
}

// The jsii proxy struct for AcknowledgementFailed
type jsiiProxy_AcknowledgementFailed struct {
	_ byte // padding
}

// Experimental.
func NewAcknowledgementFailed() AcknowledgementFailed {
	_init_.Initialize()

	j := jsiiProxy_AcknowledgementFailed{}

	_jsii_.Create(
		"@aws-cdk/mixins-preview.aws_b2bi.events.AcknowledgementFailed",
		nil, // no parameters
		&j,
	)

	return &j
}

// Experimental.
func NewAcknowledgementFailed_Override(a AcknowledgementFailed) {
	_init_.Initialize()

	_jsii_.Create(
		"@aws-cdk/mixins-preview.aws_b2bi.events.AcknowledgementFailed",
		nil, // no parameters
		a,
	)
}

// EventBridge event pattern for Acknowledgement Failed.
// Experimental.
func AcknowledgementFailed_AcknowledgementFailedPattern(options *AcknowledgementFailed_AcknowledgementFailedProps) *awsevents.EventPattern {
	_init_.Initialize()

	if err := validateAcknowledgementFailed_AcknowledgementFailedPatternParameters(options); err != nil {
		panic(err)
	}
	var returns *awsevents.EventPattern

	_jsii_.StaticInvoke(
		"@aws-cdk/mixins-preview.aws_b2bi.events.AcknowledgementFailed",
		"acknowledgementFailedPattern",
		[]interface{}{options},
		&returns,
	)

	return returns
}

