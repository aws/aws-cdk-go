package previewawsecsevents


// Type definition for NetworkConfiguration.
//
// Example:
//   // The code below shows an example of how to instantiate this type.
//   // The values are placeholders you should change.
//   import "github.com/aws/aws-cdk-go/awscdkmixinspreview"
//
//   networkConfiguration := &NetworkConfiguration{
//   	AwsvpcConfiguration: &AwsvpcConfiguration{
//   		AssignPublicIp: []*string{
//   			jsii.String("assignPublicIp"),
//   		},
//   		Subnets: []*string{
//   			jsii.String("subnets"),
//   		},
//   	},
//   }
//
// Experimental.
type ClusterEvents_AWSAPICallViaCloudTrail_NetworkConfiguration struct {
	// awsvpcConfiguration property.
	//
	// Specify an array of string values to match this event if the actual value of awsvpcConfiguration is one of the values in the array. Use one of the constructors on the `aws_events.Match`  for more advanced matching options.
	// Default: - Do not filter on this field.
	//
	// Experimental.
	AwsvpcConfiguration *ClusterEvents_AWSAPICallViaCloudTrail_AwsvpcConfiguration `field:"optional" json:"awsvpcConfiguration" yaml:"awsvpcConfiguration"`
}

