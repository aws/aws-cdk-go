...
func NewDataStoreDeleting() DataStoreDeleting {
	...
	_jsii_.Create(
		"@aws-cdk/mixins-preview.aws_healthlake.events.DataStoreDeleting",
		nil, // no parameters
		&j,
	)
...
// Experimental.
func DataStoreDeleting_EventPattern(options *DataStoreDeleting_DataStoreDeletingProps) *awsevents.EventPattern {
	_init_.Initialize()

	if err := validateDataStoreDeleting_EventPatternParameters(options); err != nil {
		panic(err)
	}
