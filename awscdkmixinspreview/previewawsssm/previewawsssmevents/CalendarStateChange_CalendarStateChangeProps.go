package previewawsssmevents

import (
	"github.com/aws/aws-cdk-go/awscdk/v2"
)

// Props type for aws.ssm@CalendarStateChange event.
//
// Example:
//   // The code below shows an example of how to instantiate this type.
//   // The values are placeholders you should change.
//   import "github.com/aws/aws-cdk-go/awscdkmixinspreview"
//
//   calendarStateChangeProps := &CalendarStateChangeProps{
//   	AtTime: []*string{
//   		jsii.String("atTime"),
//   	},
//   	EventMetadata: &AWSEventMetadataProps{
//   		Region: []*string{
//   			jsii.String("region"),
//   		},
//   		Resources: []*string{
//   			jsii.String("resources"),
//   		},
//   		Version: []*string{
//   			jsii.String("version"),
//   		},
//   	},
//   	NextTransitionTime: []*string{
//   		jsii.String("nextTransitionTime"),
//   	},
//   	State: []*string{
//   		jsii.String("state"),
//   	},
//   }
//
// Experimental.
type CalendarStateChange_CalendarStateChangeProps struct {
	// atTime property.
	//
	// Specify an array of string values to match this event if the actual value of atTime is one of the values in the array. Use one of the constructors on the `aws_events.Match`  for more advanced matching options.
	// Default: - Do not filter on this field.
	//
	// Experimental.
	AtTime *[]*string `field:"optional" json:"atTime" yaml:"atTime"`
	// EventBridge event metadata.
	// Default: - -.
	//
	// Experimental.
	EventMetadata *awscdk.AWSEventMetadataProps `field:"optional" json:"eventMetadata" yaml:"eventMetadata"`
	// nextTransitionTime property.
	//
	// Specify an array of string values to match this event if the actual value of nextTransitionTime is one of the values in the array. Use one of the constructors on the `aws_events.Match`  for more advanced matching options.
	// Default: - Do not filter on this field.
	//
	// Experimental.
	NextTransitionTime *[]*string `field:"optional" json:"nextTransitionTime" yaml:"nextTransitionTime"`
	// state property.
	//
	// Specify an array of string values to match this event if the actual value of state is one of the values in the array. Use one of the constructors on the `aws_events.Match`  for more advanced matching options.
	// Default: - Do not filter on this field.
	//
	// Experimental.
	State *[]*string `field:"optional" json:"state" yaml:"state"`
}

